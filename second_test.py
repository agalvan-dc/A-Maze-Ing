...
class ImageRenderer:
    """Renderer que estira y adapta la imagen exactamente
    ...
    def prepare_image(self, m_instance: Mlx, mlx_ptr, win_w: int, win_h: int):
        ...
        if hasattr(m_instance, 'mlx_png_file_to_image'):
            ...
            print("[ERROR] El wrapper de MLX "
                  "no ofrece métodos de carga conocidos.")
            return
    ...
    def draw(self, m_instance: Mlx, mlx_ptr, win_ptr):
        ...
        m_instance.mlx_put_image_to_window(mlx_ptr,
                                           win_ptr, self.img_ptr, 0, 0)
...
class MazeWindow:
    def __init__(self, renderer: ImageRenderer,
                 width: int = 800,
                 height: int = 600, title: str = "Image Stretched"):
        self.m = Mlx()
    ...
    def _create_window(self, w: int, h: int):
        ...
        self.renderer.prepare_image(
            self.m, self.mlx_ptr, self.width, self.height)
        self.render_frame()
    ...
    def on_key(self, keycode: int, param):
        if keycode in (65307, 53):  # ESC
            ...
            self._create_window(min(1920,
                                    int(self.width * 1.2)),
                                min(1080, int(self.height * 1.2)))
        elif keycode in (45, 65453):
            self._create_window(max(300,
                                    int(self.width * 0.8)),
                                max(300, int(self.height * 0.8)))
...
if __name__ == "__main__":
    ...
    print("Ejecutando... Usa '+' y '-' para redimensionar"
          " la ventana y la imagen al 100%.")
    app.run()
